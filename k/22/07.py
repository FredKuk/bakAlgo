dx = [1, 0, -1, 0]
dy = [0, 1, 0, -1]
def check(board:list, aloc:list, bloc:list):
    for i in range(len(dx)):
        if aloc[0]+dx[i] in [-1,len(board[0])] : return False
        if aloc[1]+dy[i] in [-1,len(board)] : return False
        if bloc[0]+dx[i] in [-1,len(board[0])] : return True
        if bloc[1]+dy[i] in [-1,len(board)] : return True

def solution(board:list, aloc:list, bloc:list):
    return game(board,al)
...
보드 안에는 발판이 있는 부분과 없는 부분이 있습니다. 발판이 있는 곳에만 캐릭터가 서있을 수 있으며, 처음 캐릭터를 올려놓는 곳은 항상 발판이 있는 곳입니다. 
캐릭터는 발판이 있는 곳으로만 이동할 수 있으며, 보드 밖으로 이동할 수 없습니다. 밟고 있던 발판은 그 위에 있던 캐릭터가 다른 곳으로 이동하여 다른 발판을 밞음과 동시에 사라집니다.
...
따라서 플레이어 A는 이길 수 있는 플레이어이며, B는 질 수밖에 없는 플레이어입니다. 다음은 A와 B가 최적의 플레이를 하는 과정을 나타냅니다.
...
만약 과정 2에서 B가 아래쪽 칸인 (2, 2)로 이동하더라도 A는 (2, 1)로 이동하면 됩니다.
 이후 B가 (2, 1)로 이동, 다음 차례에 A가 (2, 0)으로 이동하면 B가 패배합니다.
